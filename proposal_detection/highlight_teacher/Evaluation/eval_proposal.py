...
class ANETproposal(object):
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        # Checking format
        with open('../data/highlight/validation_list.json', 'r') as f:
            video_list1 = json.load(f)
        video_list1 = [video_name for video_name in video_list1]
        video_list2 = list(database.keys())
        video_list = list(set(video_list1).intersection(set(video_list2)))
        # Read ground truth data.
        video_lst, t_start_lst, t_end_lst, label_lst = [], [], [], []
        for video_name in video_list:
            for ann in database[video_name]['annotations']:
