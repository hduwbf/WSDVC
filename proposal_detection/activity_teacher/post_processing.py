...
def getDatasetDict(opt):
    database = load_json(opt["anno_path"]+'Anet_anno.json')
    video_list1 = load_json(opt['anno_path']+'validation_list.json')
...
def random_post_processing(opt):
    ...
    for video_name in video_list:
        result = []
        duration = float(video_dict[video_name]['feature_frame'])/video_dict[video_name]['duration_frame']*video_dict[video_name]['duration_second']
        for i in range(100):
            anno = {}
