...
def base_feature_loss(weight, student_feature, teacher_feature):
    weight = torch.sqrt(weight).unsqueeze(1).unsqueeze(2).expand_as(student_feature)
    loss = F.mse_loss(student_feature, teacher_feature)
    return loss

def teacher_bmn_loss_func(weight, pred_bm, pred_start, pred_end, gt_iou_map, gt_start, gt_end, bm_mask):
    ...
    tem_loss = teacher_tem_loss_func(weight, pred_start, pred_end, gt_start, gt_end)
    pem_reg_loss = teacher_pem_reg_loss_func(weight, pred_bm_reg, gt_bm_reg, bm_mask)
    pem_cls_loss = teacher_pem_cls_loss_func(weight, pred_bm_cls, gt_bm_cls, bm_mask)

    loss = tem_loss + pem_reg_loss + pem_cls_loss
    return loss, tem_loss, pem_reg_loss, pem_cls_loss

def teacher_tem_loss_func(weight, pred_start, pred_end, gt_start, gt_end):
    '''
    pred_start: bs, 100
    pred_end: bs, 100
    ...
    '''
    def bi_loss(weight, pred_score, gt_label):
        ...
        weight = weight.unsqueeze(1).expand_as(pred_score).contiguous()
        ...
        coef_1 = 0.5 * ratio
        epsilon = 0.000001
        loss_pos = coef_1 * torch.log(pred_score + epsilon) * pmask * weight
        loss_neg = coef_0 * torch.log(1.0 - pred_score + epsilon) * (1.0 - pmask) * weight
        loss = -1 * torch.mean(loss_pos + loss_neg) # (bs)
        return loss

    loss_start = bi_loss(weight, pred_start, gt_start)
    loss_end = bi_loss(weight, pred_end, gt_end)
    loss = loss_start + loss_end
    return loss

def teacher_pem_reg_loss_func(weight, pred_score, gt_iou_map, mask):
    def reg_loss(weight, pred_score, gt_iou_map, mask):
        ...
        weights = u_hmask + u_slmask + u_smmask
        weight = torch.sqrt(weight).unsqueeze(1).unsqueeze(2).expand_as(pred_score)
        loss = F.mse_loss(pred_score * weights * weight, gt_iou_map * weights * weight)
        loss = 0.5 * torch.sum(loss * torch.ones(*weights.shape).cuda()) / torch.sum(weights)
        return loss * 10

    loss = reg_loss(weight, pred_score, gt_iou_map, mask)
    return loss

def teacher_pem_cls_loss_func(weight, pred_score, gt_iou_map, mask):
    def cls_loss(weight, pred_score, gt_iou_map, mask):
        max_value = gt_iou_map.max(dim=2, keepdim=True)[0].max(dim=1, keepdim=True)[0]
        min_value = (gt_iou_map + 100 - mask * 100).min(dim=2, keepdim=True)[0].min(dim=1, keepdim=True)[0]
        threshold_value = max_value - min_value
        ...
        weight = weight.unsqueeze(1).unsqueeze(2).expand_as(pred_score)
        num_positive = torch.sum(pmask) # (bs, 100, 100)
        num_entries = num_positive + torch.sum(nmask) # (bs, 100, 100)
        ratio = num_entries / num_positive
        coef_0 = 0.5 * ratio / (ratio - 1) * mask
        coef_1 = 0.5 * ratio * mask
        epsilon = 0.000001
        loss_pos = coef_1 * torch.log(pred_score + epsilon) * pmask * weight
        loss_neg = coef_0 * torch.log(1.0 - pred_score + epsilon) * nmask * weight
        loss = -1 * torch.sum(loss_pos + loss_neg) / num_entries
        return loss

    loss = cls_loss(weight, pred_score, gt_iou_map, mask)
    return loss

def bmn_loss_func(pred_bm, pred_start, pred_end, gt_iou_map, gt_start, gt_end, bm_mask):
    if gt_iou_map.shape[1] == 2:
        gt_bm_reg = gt_iou_map[:, 0].contiguous()
        gt_bm_cls = gt_iou_map[:, 1].contiguous()
        gt_bm_reg = gt_bm_reg * bm_mask
        gt_bm_cls = gt_bm_cls * bm_mask
        ...
        pem_reg_loss = pem_reg_loss_func(pred_bm_reg, gt_bm_reg, bm_mask)
        pem_cls_loss = pem_cls_loss_func(pred_bm_cls, gt_bm_cls, bm_mask)
