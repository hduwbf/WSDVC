...
class VideoDataSet(data.Dataset):
    ...
    def _getDatasetDict(self):
        self.video_dict = {}
        self.video_list = load_json(self.anno_path + self.subset + '_list.json')
        video_database = load_json(self.anno_path + 'Anet_anno.json')
    ...
    def _get_train_label(self, index, anchor_xmin, anchor_xmax):
        ...
        for j in range(len(video_labels)):
            ...
            tmp_start = max(min(1, tmp_info['segment'][0] / duration_second), 0)
            tmp_end = max(min(1, tmp_info['segment'][1] / duration_second), 0)
...
class ActionDataSet(data.Dataset):
    def __init__(self, opt, subset="train"):
        ...
        self.subset = subset
        self.mode = opt["mode"]
        self.feature_path = "../data/action/TH14Features.hdf5"
        self.anno_path = "../data/action/"
        self._getDatasetDict()
        self.anchor_xmin = [self.temporal_gap * (i - 0.5) for i in range(self.temporal_scale)]
        self.anchor_xmax = [self.temporal_gap * (i + 0.5) for i in range(self.temporal_scale)]
    ...
    def _getDatasetDict(self):
        self.video_dict = {}
        self.video_list = load_json(self.anno_path + self.subset + '_list.json')
        video_database = load_json(self.anno_path + 'new_th14.json')
        for video_name in self.video_list:
            self.video_dict[video_name] = video_database[video_name]
        print("%s subset video numbers: %d" % (self.subset, len(self.video_list)))
...
class ActivityDataSet(data.Dataset):
    def __init__(self, opt, subset="train"):
        ...
        self.subset = subset
        self.mode = opt["mode"]
        self.feature_path = "../data/activity/AnetFeatures.hdf5"
        self.anno_path = "../data/activity/"
        self._getDatasetDict()
        self.anchor_xmin = [self.temporal_gap * (i - 0.5) for i in range(self.temporal_scale)]
        self.anchor_xmax = [self.temporal_gap * (i + 0.5) for i in range(self.temporal_scale)]
...
class HighlightDataSet(data.Dataset):
    def __init__(self, opt, subset="train"):
        self.temporal_scale = opt["temporal_scale"]  # 100
        self.temporal_gap = 1. / self.temporal_scale
        self.video_features = None
        self.subset = subset
        self.mode = opt["mode"]
        self.feature_path = "../data/highlight/BHFeatures.hdf5"
        self.anno_path = "../data/highlight/"
        self._getDatasetDict()
        self.anchor_xmin = [self.temporal_gap * (i - 0.5) for i in range(self.temporal_scale)]
        self.anchor_xmax = [self.temporal_gap * (i + 0.5) for i in range(self.temporal_scale)]
