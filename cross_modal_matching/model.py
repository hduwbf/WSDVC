...
class CootModel:
    ...
    def encode_video(self, vid_frames, vid_frames_mask, vid_frames_len,
        ...
        # print('vid_frame', vid_frames[0], vid_frames.shape)
        ...
        # print('vid_context', vid_context[0], vid_context.shape)
        # compute clip embedding
        # print('clip_frames', clip_frames[0], clip_frames.shape)
        clip_emb = self.net_video_pooler(
            clip_frames, clip_frames_mask, clip_frames_len, None)
        # print('clip_emb', clip_emb[0], clip_emb.shape)
    ...
    def encode_image(self, img_frames, img_frames_mask, img_frames_len):
        # compute video context
        img_emb = self.net_video_pooler(img_frames, img_frames_mask, img_frames_len, None)
        return img_emb

    def encode_caption(self, cap_vectors, cap_mask, cap_len):
        # compute paragraph context
        cap_emb = self.net_text_pooler(cap_vectors, cap_mask, cap_len, None)
        return cap_emb
    ...
    def to_device_fn(self, model):
        if self.use_multi_gpu:
            model = nn.DataParallel(model,device_ids=range(7))
    ...
    def __init__(self, cfg: EasyDict, feature_dim: int):
        ...
        if cfg.input_fc:
            self.input_fc = nn.Sequential(
                nn.Linear(feature_dim, cfg.input_fc_output_dim), GELU())
