...
def compute_indices(num_frames_orig: int, num_frames_target: int, is_train: bool):
...
def truncated_normal_fill(shape: Tuple[int], mean: float = 0, std: float = 1, limit: float = 2) -> torch.Tensor:
    num_examples = 8
...
def compute_retr_clip_to_sent(clip_feat, sent_feat):
    num_points = clip_feat.shape[0]
    d = np.dot(clip_feat, sent_feat.T)
    return compute_retrieval_cosine(d, num_points)

def compute_retr_sent_to_clip(clip_feat, sent_feat):
    num_points = clip_feat.shape[0]
    d = np.dot(sent_feat, clip_feat.T)
    return compute_retrieval_cosine(d, num_points)
